...
describe('Song function tests', () => {
  const song1 = new Song('Blessed and Possessed', 4.5, false, 1900000, 'Metal');
  const song2 = new Song('Mordekaiser, the Iron Revenant', 2.48, false, 2000000, 'Metal', 'Soundtrack');
  const song3 = new Song('Entre dos tierras', 6.08, true, 620000000, 'Rock', 'Pop', 'Latin')
  ...
  it('song3.genres = [\'Rock\', \'Pop\', \'Latin\']', () => {
    expect(song3.genres = ['Rock', 'Pop', 'Latin']).to.have.members(['Rock', 'Pop', 'Latin']);
  });
...
describe('CD function tests', () => {
  const song1 = new Song('Bad to the Bone', 4.5, true, 41000000, 'Rock', 'Blues');
  const cd1 = new CD('Bad to the Bone', 1987, song1);

  const song2 = new Song('Enter Hallownest', 1.29, false, 410000, 'Orchestral', 'Soundtrack');
  const song3 = new Song('Mantis Lords', 1.45, false, 567000, 'Orchestral', 'Soundtrack', 'Choral');
  const song4 = new Song('Radiance', 2.17, false, 548000, 'Orchestral', 'Soundtrack', 'Choral');

  const cd2 = new CD('Hollow Knight OST', 2017, song2, song3, song4);
  ...
  it('cd2.songs = [song3, song4]', () => {
    expect(cd2.songs = [song3, song4]).to.have.members([song3, song4]);
  });
  it('cd2.addSong(song1) adds song1 to cd2.songs', () => {
    cd2.addSong(song1);
    expect(cd2.songs).to.have.members([song3, song4, song1]);
  });
});